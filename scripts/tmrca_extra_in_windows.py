...
def tmrca_means(df):
    ...
    return pandas.DataFrame({'start': [df.start.min()],
    	                     ...
    	                     'pruned_tmrca_half': [(df.pruned_tmrca_half * segment_lengths).sum() / segment_lengths.sum()],
                             'pruned_tmrca': [(df.pruned_tmrca * segment_lengths).sum() / segment_lengths.sum()],
    	                     'pruned_coal_half': [(df.pruned_coal_half * segment_lengths).sum() / segment_lengths.sum()]})
