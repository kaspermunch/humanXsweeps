...
def indiv_filter(df):
    indiv1_in_meta = [x in list(individuals) + ['Altai', 'Denisova'] for x in df.indiv_1]
    indiv2_in_meta = [x in list(individuals) + ['Altai', 'Denisova'] for x in df.indiv_2]
...
def swap_indiv_to_put_archaic_last(s1, s2):
    l = list()
    for indiv1, indiv2 in zip(s1, s2):
        if indiv1 in ['Altai', 'Denisova']:
            l.append((indiv2, indiv1))
        else:
            l.append((indiv1, indiv2))
    return l

dist_data['indiv_1'], dist_data['indiv_2'] = zip(*swap_indiv_to_put_archaic_last(dist_data['indiv_1'], dist_data['indiv_2']))
